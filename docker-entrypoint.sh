...
docker_temp_server_start() {
	if [ "${MYSQL_MAJOR}" = '5.7' ]; then
		"$@" --skip-networking --default-time-zone=${TIMEZONE:-SYSTEM} --socket="${SOCKET}" &
		...
		if ! "$@" --daemonize --skip-networking --default-time-zone=${TIMEZONE:-SYSTEM} --socket="${SOCKET}"; then
			mysql_error "Unable to start server."
		fi
	fi
}
...
docker_init_database_dir() {
	...
	"$@" --initialize-insecure --default-time-zone=${TIMEZONE:-SYSTEM}
